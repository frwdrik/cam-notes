...
\begin{thm}[Eisentein criterion]\index{Eisentein criterion}
...
\begin{defi}[Eisentein polynomial]\index{Eisentein polynomial}
  A polynomial $f(x) \in \mathcal{O}_K[x]$ satisfying the assumptions of Eisentein's criterion is called an \term{Eisentein polynomial}.
\end{defi}
...
\begin{defi}[$i_{L/K}$]\index{$i_{L/K}$} % does this have a name?
  We define
  \[
...
\begin{lemma}
  ...
  \[
    L = \bigcup_m L_m. % find out correct notation
  \]
  Then we have
  \[
...
\]
So it follows that $L \subseteq \Q_{p^n}(\zeta_{p^m})$ for some $n$ if and only if $G^s(L/\Q_p) = 1$ for all $s > m - 1$. % why?
